...
## Updated by Alyssa R. Hamm, May 2025. Made formatting changes, and changed FeatureCounts from "directional" to "unique"
...
then
	...
	python $dir/featureCounts_unique_5.py ${sample} ${custom_name} ${sample} ${gff} # annotate features from gff and identify UMI groups
	...
	#rm -r ${custom_name}_no_XT # sam and bam files without XT tag (generated during featureCounts_unique_5)
	#rm -r ${custom_name}_FC # bam files after feature counts (featureCounts_unique_5)
	#rm -r ${custom_name}_FC_directional_grouped_2 # bam files after umi tools group (featureCounts_unique_5)
	echo $msg
fi
